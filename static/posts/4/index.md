...
Windows는 개발자 친화적인 OS는 아닙니다. Windows에 특화된 생태계는 크고 복잡하여 쉽게 들어설 수 없는 거대한 장벽이 있어 소위 말하는 일반적인 개발 경험과 다소 거리가 있습니다. 저는 학부생 시절부터 Windows와 WSL (Windows Subsystem for Linux)로 소프트웨어 개발을 시작했지만 Windows에서 벗어날수록 개발이 편해졌다는 건 결코 부인할 수 없습니다.

개발 환경이 그럴진대, 서버 애플리케이션을 Windows에 배포하는 것은 어떨까요? 가상화 및 컨테이너 기술이 발전하면서 제 주변에서 Linux 외의 환경에 서버 애플리케이션을 배포하는 것은 흔히 보기 어려워졌습니다. 어디서든 Linux 환경에서 개발할 수 있게 되었으니까요.

이전 회사에서는 서비스 운영을 위해 수많은 웹사이트에서 데이터를 스크래핑해야 했습니다. 대부분의 데이터는 Linux 환경에서 수집할 수 있었지만 드물게 Windows 환경이 강제되는 경우도 있었습니다. 이번 글에서는 그 경험을 공유하려고 합니다.
...
보안 프로그램을 우회하는 방법은 여럿 있지만 가장 쉬운 것은 다른 OS를 이용해 보는 것입니다. 일부 사이트는 Windows를 위해 구현된 보안 프로그램을 이용할 수 없는 OS에서는 보안 프로그램 없이도 로그인할 수 있도록 허용하기도 합니다. 혹은 사이트 결함(단순히 ESC 키를 누른다거나)으로 설치 요구를 무시할 수 있는 경우도 있습니다.
...
<figure class="image">
  ...
  <figcaption>https://skensita.tistory.com/entry/키보드-보안</figcaption>
</figure>
...
보안 프로그램을 뜯어다 암호화 알고리즘과 키를 찾아내어 직접 암호화를 수행하는 방법입니다. 하지만 프로그램을 직접 뜯어보는 것은 난이도가 높고, 또한 보안 프로그램이 업데이트되면 암호화 알고리즘과 키가 변경될 수 있어 유지보수가 어렵습니다.

### 🔌 하드웨어 입력 자동화하기

하드웨어 입력을 소프트웨어로 제어할 수 있는 USB 장치를 직접 만들어 문제를 해결하는 개발자도 있다고 합니다. 하지만 제겐 그러한 지식이 없고, 모든 서비스는 AWS 클라우드에 호스팅되어 있어 이 장치를 마련하더라도 연결할 수 있는 물리 서버도 없었습니다.

서버를 구매하고, 하드웨어를 제작 또는 외주를 통해 조달하는 등 기능의 중요도에 비해 필요로 하는 비용이 너무 높았습니다.

### 👁️ 정직하게 구현하기

브라우저 스크린샷과 OpenCV 템플릿 매칭을 활용하여 키보드 버튼을 찾아 클릭하는 방법입니다. 보안 키보드의 변경에 취약할 수 있고 보안 키보드의 크기, 위치, 색상 등은 사이트마다 다를 수 있습니다.

하지만 보안 키패드의 키 이미지는 자주 변경되는 요소는 아닙니다. 거기다 추가 비용이 들지 않고, 소프트웨어적으로 구현할 수 있는 방법입니다.

## ✨ 로그인 자동화 구현하기

앞서 다양한 선택지를 검토한 끝에 결국 보안 키보드를 성실하게 클릭하는 방법이 가장 합리적인 선택이 되었습니다. [soulee-dev/fuckvkeypad](https://github.com/soulee-dev/fuckvkeypad)에서 영감을 얻어 OpenCV 템플릿 매칭을 이용해 보안 키보드 버튼을 찾아 클릭하는 기능을 구현했습니다.
...
- 그 외 VPC, NAT, CloudWatch, IAM 등 많은 요소가 직간접적으로 연관되어 있습니다. 당장은 동작하는 구성을 실험하기 위해 HTTPS (Route 53 / ACM)는 구성하지 않았습니다.
...
이제 실험을 통해 구축한 인프라가 잘 동작하는지 확인해보겠습니다.

### 🛡️ 보안 프로그램 동작 확인

Session Manager를 이용하여 EC2 인스턴스에 접속한 뒤, 보안 프로그램이 정상적으로 동작하는지 확인합니다.

![보안 프로그램 동작 확인](./assets/security-program-check.png)

이제 RDP 세션 연결 및 종료 후, 콘솔 세션이 자동으로 시작되는지 확인해보겠습니다.

먼저 Fleet Manager를 이용하여 RDP로 접속합니다.

![Fleet Manager - Remote Desktop](./assets/fleet-manager-remote-desktop.png)

![RDP 세션 확인](./assets/rdp-session-check.png)

RDP 세션이 정상적으로 생성되며, 기존 콘솔 세션이 방해받지 않습니다. 세션을 끊은 뒤, 잠시 후 다시 확인해보겠습니다.

![콘솔 세션 확인](./assets/console-session-check.png)
...
잘 배포되었는지 CodeDeploy 콘솔을 확인해보겠습니다.

![CodeDeploy 배포 확인](./assets/codedeploy-deployment-check.png)
...
실험을 통해 파악한 여러 시나리오 별 소요 시간은 대략적으로 다음과 같았습니다.
...
일반적인 수준의 크롤러 애플리케이션이라면 다양한 라이브러리, Selenium 웹 드라이버 등 많은 의존성을 가지므로 최초 배포 시 적어도 5분은 걸릴 것이라고 예상하고 있습니다. 필요하다면 이러한 내부 의존성 설치 속도를 올리기 위한 내부 미러링(pip / Selenium 웹 드라이버 등) 또한 고려할 수 있을 것 같네요. 또한 적절한 EC2 인스턴스 유형을 선택하면 초기화 및 의존성 설치 속도를 개선하여 배포 속도를 더 올릴 수 있을 것입니다.
