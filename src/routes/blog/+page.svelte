<script lang="ts">
	import { formatDistance } from 'date-fns';

	export let data;

	const { allPosts } = data;
	const allTags = new Set(allPosts.map((post) => post.metadata.tags).flat());
</script>

<div>
	<div class="grid grid-cols-1">
		<!-- TODO: Pagination -->
		<!-- TODO: Search posts -->
		<section data-testid="tags">
			<h2 class="mb-3 text-3xl">Tags</h2>
			<div
				class="max-sm:flex-cols space-x-1 max-sm:flex max-sm:overflow-x-scroll max-sm:pb-2 md:space-y-1"
			>
				{#if allTags.size}
					{#each allTags as tag}
						<div class="badge badge-neutral badge-sm p-3 font-semibold md:badge-md">
							<a href="/blog/tag/{tag}">
								{tag}
							</a>
						</div>
					{/each}
				{:else}
					<p class="text-lg">There is no tag yet.</p>
				{/if}
			</div>
		</section>
		<section data-testid="posts" class="mt-7">
			<h2 class="mb-3 text-3xl">Posts</h2>

			<!-- Post grid -->
			<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3" role="rowgroup">
				{#if allPosts.length}
					{#each allPosts as { id, metadata: { title, publicationDate, tags } }}
						<!-- Post card-->
						<div
							class="lg:h-30 card card-compact m-2 rounded-lg shadow-lg md:card-normal"
							role="cell"
						>
							<!-- Preview image -->
							<a href="/blog/{id}">
								<figure>
									<img
										src="data:image/png;base64,"
										alt="Preview"
										class="h-30 w-full rounded-lg"
									/>
								</figure>
							</a>

							<!-- Content body -->
							<div class="card-body">
								<a href="/blog/{id}">
									<h2 class="card-title">{title}</h2>
									<p class="mb-2 text-right">
										<time datetime={publicationDate.toISOString()} role="time"
											>{formatDistance(publicationDate, new Date(), { addSuffix: true })}</time
										>
									</p>
								</a>

								<!-- Badges -->
								<div class="flex-cols flex overflow-x-scroll p-1 pb-2">
									{#each tags as tag}
										<div class="badge badge-neutral mr-1 text-xs">
											<a href="/blog/tag/{tag}">{tag}</a>
										</div>
									{/each}
								</div>
							</div>
						</div>
					{/each}
				{:else}
					<p class="text-lg">There is no post yet.</p>
				{/if}
			</div>
		</section>
	</div>
</div>
